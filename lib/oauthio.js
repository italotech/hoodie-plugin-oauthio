...
module.exports = function (hoodie, pluginDb) {
  ...
  var _getOauthConfig = function (task, cb) {
    log2('getOauthConfig->_getOauthConfig', task);    
    try {
  ...
  var _verifyUser = function (task, cb) {
    log2('verifyUser->_verifyUser', task);    
    var doc = task.me.raw;
  ...
  return {
    ...
    verifyUser: function (db, task) {
      ...
      async.series([
       // async.apply(generatePassword, task),
        async.apply(_verifyUser, task),
      ], utils.handleTask(hoodie, 'verifyUser', db, task));
